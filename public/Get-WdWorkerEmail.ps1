...
Get-WdWorkerEmail -WorkerId 123
...
#>
    ...
    if ($PsCmdlet.ParameterSetName -eq 'Search') {
        $response = Get-WdWorker -WorkerId $WorkerId -WorkerType $WorkerType -IncludePersonal -Passthru -Human_ResourcesUri $Human_ResourcesUri -Username:$Username -Password:$Password -IncludeInactive:$IncludeInactive -ErrorAction Stop
