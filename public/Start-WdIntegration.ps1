...
.PARAMETER Integrations_ResourcesUri
    ...
    stored with Set-WdEndpoint -Endpoint Integration is tried.
...
.EXAMPLE
    Start-WdIntegration -Id Integration/Id/StartHere -Type Integration_System_ID -Integrations_ResourcesUri 'https://SERVICE.workday.com/ccx/service/TENANT/Integrations/v26.0'
...
'@
    ...
    $response = Invoke-WdRequest -Request $request -Uri $Integrations_ResourcesUri -Username:$Username -Password:$Password
    ...
    $output.Xml = $response.Xml
    if ($Wait) {
        $wie = Get-WdIntegrationEvent -Wid $output.Wid -Integrations_ResourcesUri:$Integrations_ResourcesUri -Username:$Username -Password:$Password
