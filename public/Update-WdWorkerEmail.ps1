...
.DESCRIPTION
    ...
    the email is the same. Unlike Set-WdWorkerEmail, this cmdlet
...
Update-WdWorkerEmail -WorkerId 123 -Email test@example.com

.NOTES
    The Set-WdWorkerEmail switch -Append is not supported, as the -Secondary
...
#>
    ...
    if ($PsCmdlet.ParameterSetName -eq 'NoSearch') {
        $current = Get-WdWorkerEmail -WorkerXml $WorkerXml
        $WorkerType = 'WID'
        ...
        $current = Get-WdWorkerEmail -WorkerId $WorkerId -WorkerType $WorkerType -Human_ResourcesUri:$Human_ResourcesUri -Username:$Username -Password:$Password -IncludeInactive:$IncludeInactive
    }
    ...
    if (
        ...
        $o = Set-WdWorkerEmail -WorkerId $WorkerId -WorkerType $WorkerType -Email $Email -UsageType:$UsageType -Private:$Private -Secondary:$Secondary -Human_ResourcesUri:$Human_ResourcesUri -Username:$Username -Password:$Password
        if ($null -ne $o) {
            if ($o.Success) {
                $output.Success = $true
