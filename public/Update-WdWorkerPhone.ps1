...
.DESCRIPTION
    ...
    the number is the same. Unlike Set-WdWorkerPhone, this cmdlet
...
Update-WdWorkerPhone -WorkerId 123 -Number 1234567890
...
#>
    ...
    if ($PsCmdlet.ParameterSetName -eq 'NoSearch') {
        $current = Get-WdWorkerPhone -WorkerXml $WorkerXml
        $WorkerType = 'WID'
        ...
        $current = Get-WdWorkerPhone -WorkerId $WorkerId -WorkerType $WorkerType -Human_ResourcesUri:$Human_ResourcesUri -Username:$Username -Password:$Password -IncludeInactive:$IncludeInactive
    }
    ...
    if (
        ...
        Write-Debug $params
        $o = Set-WdWorkerPhone -WorkerId $WorkerId -WorkerType $WorkerType @params
        if ($null -ne $o) {
            if ($o.Success) {
                $output.Success = $true
