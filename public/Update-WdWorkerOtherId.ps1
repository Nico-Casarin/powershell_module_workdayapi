function Update-WdWorkerOtherId {
<#
.SYNOPSIS
...
.DESCRIPTION
    ...
    the values are the same. Unlike Set-WdWorkerOtherId, this cmdlet
...
#>
    ...
    if ($PsCmdlet.ParameterSetName -eq 'NoSearch') {
        $otherIds = Get-WdWorkerOtherId -WorkerXml $WorkerXml
        $WorkerType = 'WID'
        ...
        $otherIds = Get-WdWorkerOtherId -WorkerId $WorkerId -WorkerType $WorkerType -Human_ResourcesUri:$Human_ResourcesUri -Username:$Username -Password:$Password -IncludeInactive:$IncludeInactive
    }
    ...
    if (
        ...
        $o = Set-WdWorkerOtherId @params -Human_ResourcesUri:$Human_ResourcesUri -Username:$Username -Password:$Password -IssuedDate:$IssuedDate -ExpirationDate:$ExpirationDate
        if ($null -ne $o) {
            if ($o.Success) {
                $output.Success = $true
