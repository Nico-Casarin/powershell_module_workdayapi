...
.PARAMETER Passthru
    Outputs Invoke-WdRequest object, rather than a custom Worker object.
...
'@
        ...
        $csvFilePath = Join-Path $userHome "worker_data.csv"
        $headers = "WorkerWid", "WorkerDescriptor", "WorkerType", "WorkerId"
        $headers -join "," | Set-Content -Path $csvFilePath -Encoding UTF8
        ...
        $WorkdayData = Import-Csv -Path $csvFilePath | select-object * -ExcludeProperty WorkerWid 
    }
}
