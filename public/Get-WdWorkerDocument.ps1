...
Get-WdWorkerDocument -WorkerId 123
...
#>
    ...
    if ($PsCmdlet.ParameterSetName -eq 'Search') {
        $response = Get-WdWorker -WorkerId $WorkerId -WorkerType $WorkerType -IncludeInactive -IncludeDocuments -PassThru -Human_ResourcesUri $Human_ResourcesUri -Username:$Username -Password:$Password -ErrorAction Stop
        if (-not $response.Success) { throw "Error getting documents using Get-WdWorker: $($response.Message)" }
